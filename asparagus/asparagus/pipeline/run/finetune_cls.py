...
OmegaConf.register_new_resolver(
    "version",
    lambda resume_training, run_dir: generate_unused_run_id(
        resume_training=resume_training, run_dir=run_dir
    ),
    use_cache=True,
)
...
@hydra.main(config_path=get_config_path(), config_name="default_finetune_cls", version_base="1.2")
def main(cfg: DictConfig) -> None:
    print(f"{OmegaConf.to_yaml(cfg)}\n Version: {cfg.run_id}\n Run dir: {HydraConfig.get().run.dir}\n")
    safe_cfg = OmegaConf.to_container(cfg, resolve=True, throw_on_missing=True)
    ...
    cpu_train = instantiate(cfg.transforms._cpu_tr_transforms, target_size=cfg.training.target_size)
    cpu_val = instantiate(cfg.transforms._cpu_val_transforms, target_size=cfg.training.target_size)
    gpu_train = instantiate(cfg.transforms._gpu_tr_transforms, ndim=len(cfg.training.target_size))
    data_module = instantiate(
        cfg.lightning._data_module,
        train_split=file_store.splits["train"],
        val_split=file_store.splits["val"],
        train_transforms=cpu_train,
        val_transforms=cpu_val,
    ...
    module = instantiate(
        ...
        test_output_path=os.path.join(
            path_store.run_dir,
            "predictions",
            cfg.test_task + "__" + cfg.data.test_split + "__best.json",
        ),
    )
    trainer = instantiate(
    ...
    validation = trainer.validate(model=module, datamodule=data_module, ckpt_path=best.best_model_path)
    if trainer.is_global_zero:
        save_json(validation[0], os.path.join(path_store.run_dir, "validation_best_metrics.json"))
    module.model.apply(set_params_to_zero)
    trainer.test(model=module, datamodule=data_module, ckpt_path=best.best_model_path)
