...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fomo26_contract_") as temporary:
        ...
        run(sifs["task2"], ["--flair", "/input/flair.nii.gz", "--dwi", "/input/dwi.nii.gz", "--swi", "/input/swi.nii.gz", "--output", f"/output/{task2_out.name}", "--no-tta"], t2, task2_out, args.timeout)
        rows["task2"] = nifti_check(task2_out, t2 / "flair.nii.gz", {0, 1})

        t3 = args.input_root / "task3/case_001"
        task3_out = out / "task3.txt"
        run(sifs["task3"], ["--t1", "/input/t1.nii.gz", "--output", f"/output/{task3_out.name}", "--no-tta"], t3, task3_out, args.timeout)
        ...
        run(sifs["task4"], ["--t2", "/input/t2.nii.gz", "--output", f"/output/{task4_out.name}", "--no-tta"], t4, task4_out, args.timeout)
        rows["task4"] = nifti_check(task4_out, t4 / "t2.nii.gz", {0, 1, 2})

        t5 = args.input_root / "task5/case_001"
        task5_out = out / "task5.txt"
        run(sifs["task5"], ["--t1", "/input/t1.nii.gz", "--output", f"/output/{task5_out.name}", "--no-tta"], t5, task5_out, args.timeout)
        ...
        for case in ("case_001", "case_002"):
            ...
            run(sifs["task6"], ["--input", "/input/input.nii.gz", "--output", f"/output/{target.name}", "--no-tta"], source, target, args.timeout)
